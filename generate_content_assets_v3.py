...
OUTPUT_ROOT = Path(r"D:\Desktop\Apps\Video Transcriber\output\content_v3")
...
SKIP_EXISTING = True
MAX_VIDEO_TRANSCRIPT_CHARS = 12000
MAX_COURSE_TRANSCRIPT_CHARS = 22000
...
VIDEO_SCHEMA = {
    ...
    "required": [
        ...
        "instagram_carousels", "short_video_scripts", "newsletter_draft",
        "lead_magnet_idea", "image_prompts"
    ]
}
...
def build_video_prompt(title: str, transcript: str) -> str:
    return f"""
You are a senior content strategist and brand ghostwriter.
...
- Your job is to extract transferable principles and intelligently map them into useful
  Abu Dhabi property advisory content where relevant.
- Do not force bad analogies.
- Keep the tone premium, calm, analytical, and trustworthy.
...
def render_video_md(data: dict[str, Any]) -> str:
    ...
    lines += ["## Newsletter Draft", data.get("newsletter_draft", ""), "", "## Lead Magnet Idea", data.get("lead_magnet_idea", ""), "", "## Image Prompts"]
...
def render_course_md(data: dict[str, Any]) -> str:
    ...
    for item in data.get("pillar_mapping", []):
        lines += [
            f"- **{item.get('pillar', '')}**: {item.get('angle', '')}"
        ]
...
def render_master_md(data: dict[str, Any]) -> str:
    ...
    for item in data.get("editorial_plan", []):
        lines += [
            f"- **{item.get('content_type', '')}** | {item.get('topic', '')} | Goal: {item.get('goal', '')}"
        ]
